...
export async function POST(request: NextRequest) {
  try {
    ...
    if (apiKey && !apiKey.startsWith('****')) {
      updates.push(
        query(`
          ...
          VALUES ('google_translate_api_key', $1)
          ON CONFLICT (key) 
    ...
    updates.push(
      query(`
        INSERT INTO settings (key, value) 
        VALUES ('google_translate_default_source_lang', $1)
        ON CONFLICT (key) 
        DO UPDATE SET value = $1
      `, [defaultSourceLang]),
      query(`
        INSERT INTO settings (key, value) 
        VALUES ('google_translate_default_target_langs', $1)
        ON CONFLICT (key) 
        DO UPDATE SET value = $1
      `, [JSON.stringify(defaultTargetLangs)]),
      query(`
        INSERT INTO settings (key, value) 
        VALUES ('google_translate_auto_translate_on_create', $1)
        ON CONFLICT (key) 
        DO UPDATE SET value = $1
      `, [autoTranslateOnCreate.toString()])
    )
...
export async function PUT(request: NextRequest) {
  try {
    ...
    try {
      ...
      logger.error('API key test error', {
        error: error instanceof Error ? error.message : 'Unknown error',
        stack: error instanceof Error ? error.stack : undefined,
    ...
    logger.error('API key test processing error', {
      error: error instanceof Error ? error.message : 'Unknown error',
      stack: error instanceof Error ? error.stack : undefined
    })
    return NextResponse.json(
