...
// GET: featured-products 섹션 가져오기
export async function GET(request: NextRequest) {
  try {
    const result = await query(`
      SELECT * FROM ui_sections 
      WHERE key = 'featured-products' OR type = 'featured'
    ...
    if (result.rows.length === 0) {
      return NextResponse.json({ 
        section: null,
        message: 'Featured products section not found'
      });
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (autoTranslate && Object.keys(translations).length > 0) {
      await saveToLanguagePack('featured-products', { ...translations, ko: content });
    }
    ...
    if (existingResult.rows.length > 0) {
      ...
      const insertResult = await query(`
        ...
        RETURNING *
      `, ['featured-products', 'featured', '이달의 특가', 5, visible, contentJson, translationsJson]);
