  ...
  try {
    ...
    if (autoTranslate) {
      ...
      if (content?.slides) {
        ...
        // 영어 번역
        translations.en.slides = await Promise.all(content.slides.map(async (slide: unknown) => ({
          ...slide,
        ...
        // 일본어 번역
        translations.jp.slides = await Promise.all(content.slides.map(async (slide: unknown) => ({
          ...slide,
      ...
      if (content?.categories) {
        ...
        translations.en.categories = await Promise.all(content.categories.map(async (cat: unknown) => ({
          ...cat,
          name: cat.name ? await translateText(cat.name, 'ko', 'en') : cat.name,
          badge: cat.badge ? await translateText(cat.badge, 'ko', 'en') : cat.badge
        })));

        translations.jp.categories = await Promise.all(content.categories.map(async (cat: unknown) => ({
          ...cat,
      ...
      if (content?.links) {
        ...
        translations.en.links = await Promise.all(content.links.map(async (link: unknown) => ({
          ...link,
          title: link.title ? await translateTagText(link.title, 'ko', 'en') : link.title
        })));

        translations.jp.links = await Promise.all(content.links.map(async (link: unknown) => ({
          ...link,
