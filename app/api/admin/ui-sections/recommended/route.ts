...
// GET: recommended 섹션 가져오기
export async function GET(request: NextRequest) {
  try {
    const result = await query(`
      SELECT * FROM ui_sections 
      WHERE key = 'recommended' OR type = 'recommended'
    ...
    if (result.rows.length === 0) {
      return NextResponse.json({ 
        section: null,
        message: 'Recommended section not found'
      });
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (autoTranslate && Object.keys(translations).length > 0) {
      await saveToLanguagePack('recommended', { ...translations, ko: content });
    }
    ...
    if (existingResult.rows.length > 0) {
      ...
      const insertResult = await query(`
        ...
        RETURNING *
      `, ['recommended', 'recommended', '추천 콘텐츠', 6, visible, contentJson, translationsJson]);
