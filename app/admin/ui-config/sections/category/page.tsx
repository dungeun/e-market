  ...
  const loadLanguages = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (data.languages) {
          setSupportedLanguages(data.languages.filter((lang: any) => lang.enabled));
        }
      }
    } catch (error) {
      ...
      if (response.ok) {
        ...
        if (data.section) {
          if (data.section.content?.categories) {
            ...
            // 기본 카테고리 설정 (메인 페이지와 일치)
            setCategoryMenus([
              { id: '1', name: '전자제품', categoryId: 'electronics', visible: true, order: 1, icon: '📱' },
              { id: '2', name: '패션', categoryId: 'fashion', visible: true, order: 2, icon: '👗', badge: 'HOT' },
              { id: '3', name: '도서', categoryId: 'books', visible: true, order: 3, icon: '📚' },
              { id: '4', name: '스포츠', categoryId: 'sports', visible: true, order: 4, icon: '⚽' },
              { id: '5', name: '뷰티', categoryId: 'beauty', visible: true, order: 5, icon: '💄' },
              { id: '6', name: '교육', categoryId: 'education', visible: true, order: 6, icon: '🎓' },
              { id: '7', name: '생활용품', categoryId: 'living', visible: true, order: 7, icon: '🏠' },
              { id: '8', name: '건강', categoryId: 'health', visible: true, order: 8, icon: '❤️' },
            ]);
          }
        ...
        body: JSON.stringify({
          content: { 
            categories: categoryMenus
              .filter(cat => cat.visible) // 보이는 카테고리만
              .map(cat => ({
                id: cat.id,
                name: cat.name,
                icon: cat.icon,
                link: `/products?category=${cat.categoryId}`,
                badge: cat.badge,
                color: getColorForCategory(cat.categoryId) // 색상 매핑 함수
              })),
            gridLayout 
          },
            ...
            <div key={category.id} className="border rounded-lg p-4 bg-gray-50">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <div className="mb-3">
                    <ImageUploadField
                      value={category.icon && category.icon.startsWith('http') ? category.icon : ''}
                      onChange={(url) => handleUpdateCategory(category.id, { icon: url })}
                      onDelete={() => handleUpdateCategory(category.id, { icon: '' })}
                      label="이미지 업로드 (추천):"
                      showPreview={false}
                      className="text-xs"
                    />
                  ...
                  {/* 기본 아이콘 선택 */}
                  <div className="mb-3">
                    <p className="text-xs font-medium text-gray-700 mb-2">기본 아이콘 선택:</p>
                    <div className="grid grid-cols-4 gap-2">
                      {[
                        ...
                        <button
                          key={item.emoji}
                          onClick={() => handleUpdateCategory(category.id, { icon: item.emoji })}
                  ...
                  <div className="flex items-center my-2">
                    ...
                    <span className="px-2 text-xs text-gray-500">또는 직접 입력</span>
                    <div className="flex-1 h-px bg-gray-300"></div>
                  </div>
                  ...
                  <div>
                    <input
                      ...
                      className="w-full px-3 py-1.5 text-xs border rounded focus:ring-2 focus:ring-blue-500"
                      placeholder="이모지 또는 텍스트 직접 입력"
                    />
                  </div>
                </div>
