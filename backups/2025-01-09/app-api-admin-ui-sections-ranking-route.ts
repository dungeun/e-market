...
export async function GET(request: NextRequest) {
  try {
    const result = await query(`
      ...
      WHERE key = 'ranking' OR type = 'ranking'
    ...
    if (result.rows.length === 0) {
      return NextResponse.json({ 
        section: null,
        message: 'Ranking section not found'
      });
    }
    ...
    console.error('Error fetching ranking section:', error);
    return NextResponse.json({ 
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (autoTranslate && Object.keys(translations).length > 0) {
      await saveToLanguagePack('ranking', { ...translations, ko: content });
    }
    ...
    if (existingResult.rows.length > 0) {
      ...
      const insertResult = await query(`
        ...
        RETURNING *
      `, ['ranking', 'ranking', '실시간 랭킹', 5, visible, contentJson, translationsJson]);
    ...
    console.error('Error updating ranking section:', error);
    return NextResponse.json({ 
