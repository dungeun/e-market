...
const fetchLanguageSections = async (language: LanguageCode): Promise<UILocalizationData> => {
  try {
    ...
    console.log(`Successfully loaded ${language} sections data`);
    return result;
  } catch (error) {
    console.error(`Failed to fetch ${language} sections:`, error);
    throw error;
  }
};
...
export function useOptimizedUILocalization(
  ...
  const finalData = useMemo(() => {
    ...
    if (error && fallbackData && language !== fallbackLanguage) {
      console.warn(`Using fallback language ${fallbackLanguage} for ${language}`);
      return fallbackData;
    }
    return undefined;
...
export function useOptimizedUILocalizationAdmin(
  ...
  const refreshData = async () => {
    try {
      ...
      console.log('Admin UI localization data refreshed');
    } catch (error) {
      console.error('Failed to refresh admin UI localization data:', error);
    }
  };
  ...
  const invalidateCache = () => {
    ...
    result.refetch();
    console.log('Admin UI localization cache invalidated');
  };
