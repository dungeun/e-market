...
export async function GET(request: NextRequest) {
  try {
    const result = await query(`
      ...
      WHERE key = 'category' OR type = 'category'
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (autoTranslate && Object.keys(translations).length > 0) {
      await saveToLanguagePack('category', { ...translations, ko: content });
    }
    ...
    if (existingResult.rows.length > 0) {
      ...
      const insertResult = await query(`
        ...
        RETURNING *
      `, ['category', 'category', '카테고리 메뉴', 2, visible, contentJson, translationsJson]);
