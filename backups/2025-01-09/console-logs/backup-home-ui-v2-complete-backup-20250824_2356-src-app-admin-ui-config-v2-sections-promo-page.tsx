...
export default function PromoSectionEditPage() {
  ...
  const loadSection = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/api/admin/homepage-sections?sectionId=promo`, {
        credentials: "include"
      });
      ...
      if (response.ok) {
        ...
        if (data.section) {
          // data 데이터를 promoBanner 상태로 설정
          if (data.section.data) {
            setPromoBanner({
              title: (typeof data.section.data.title === 'object' ? data.section.data.title.ko : data.section.data.title) || "",
              subtitle: (typeof data.section.data.subtitle === 'object' ? data.section.data.subtitle.ko : data.section.data.subtitle) || "",
              link: data.section.data.link || "",
              icon: data.section.data.icon || "",
              backgroundColor: data.section.data.backgroundColor || "#FEF3C7",
              textColor: data.section.data.textColor || "#000000",
              backgroundImage: data.section.data.backgroundImage || data.section.data.image || "",
              visible: true,
            });
          }
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/admin/homepage-sections`, {
        ...
        body: JSON.stringify({
          ...
          data: {
            title: {
              ko: promoBanner.title,
              en: promoBanner.title,
              jp: promoBanner.title,
            },
            subtitle: {
              ko: promoBanner.subtitle,
              en: promoBanner.subtitle,
              jp: promoBanner.subtitle,
            },
            icon: promoBanner.icon,
            image: promoBanner.backgroundImage,
            link: promoBanner.link,
            backgroundColor: promoBanner.backgroundColor,
            textColor: promoBanner.textColor,
          },
        }),
      });
      ...
      if (response.ok) {
        alert("저장되었습니다.");
        router.push("/admin/ui-config-v2?tab=sections");
      } else {
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button
                onClick={() => router.push("/admin/ui-config-v2?tab=sections")}
