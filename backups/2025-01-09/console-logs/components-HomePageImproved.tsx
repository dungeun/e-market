...
function HomePageImproved({
  ...
  // 히어로 슬라이드 자동 전환
  useEffect(() => {
    console.log('🎯 useEffect triggered, currentLanguage:', currentLanguage);
    console.log('🎯 sections state:', sections);
    console.log('🎯 uiLoading state:', uiLoading);
    
    const heroData = getSectionData('hero');
    console.log('🎯 heroData from getSectionData:', heroData);
    
    const slides = heroData?.slides;
    
    console.log('🎯 Hero slides data:', slides, 'length:', slides?.length);
    
    if (slides && slides.length > 1) {
      console.log('🎯 Starting auto-slide interval for', slides.length, 'slides');
      ...
      console.log('🎯 Not starting auto-slide: slides count =', slides?.length);
    }
  ...
  const renderHeroSection = useMemo(() => {
    const heroData = getSectionData('hero');
    console.log('🎯 renderHeroSection - heroData:', heroData);
    console.log('🎯 renderHeroSection - heroData.slides:', heroData?.slides);
    
    if (!heroData?.slides) {
      console.log('🎯 renderHeroSection - returning null, no heroData or slides');
      return null;
    }
