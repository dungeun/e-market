...
import { GripVertical, Eye, EyeOff, RefreshCw, Globe } from 'lucide-react';
...
export function SectionOrderTab() {
  ...
  const getSectionInfo = () => {
    const translations = {
      ko: {
        hero: { name: '히어로 배너', description: '메인 배너 슬라이드' },
        category: { name: '카테고리 메뉴', description: '카테고리별 아이콘 그리드' },
        quicklinks: { name: '바로가기 링크', description: '빠른 접근 링크' },
        promo: { name: '프로모션 배너', description: '이벤트 및 공지 배너' },
        ranking: { name: '실시간 랭킹', description: '인기/재고/검색 랭킹' },
        recommended: { name: '추천 상품', description: '큐레이션된 상품 목록' },
        products: { name: '상품 섹션', description: '상품 목록 표시' },
        'active-campaigns': { name: '진행중인 캠페인', description: '활성 캠페인 목록' },
        cta: { name: '하단 CTA', description: '회원가입 유도 영역', fixed: true }
      },
      en: {
        hero: { name: 'Hero Banner', description: 'Main banner slides' },
        category: { name: 'Category Menu', description: 'Category icon grid' },
        quicklinks: { name: 'Quick Links', description: 'Quick access links' },
        promo: { name: 'Promo Banner', description: 'Events and announcements' },
        ranking: { name: 'Real-time Ranking', description: 'Popular/Low Stock/Search rankings' },
        recommended: { name: 'Recommended', description: 'Curated product list' },
        products: { name: 'Products Section', description: 'Product list display' },
        'active-campaigns': { name: 'Active Campaigns', description: 'Active campaign list' },
        cta: { name: 'Bottom CTA', description: 'Sign up call-to-action', fixed: true }
      },
      jp: {
        hero: { name: 'ヒーローバナー', description: 'メインバナースライド' },
        category: { name: 'カテゴリーメニュー', description: 'カテゴリーアイコングリッド' },
        quicklinks: { name: 'クイックリンク', description: 'クイックアクセスリンク' },
        promo: { name: 'プロモバナー', description: 'イベントとお知らせ' },
        ranking: { name: 'リアルタイムランキング', description: '人気/在庫/検索ランキング' },
        recommended: { name: 'おすすめ商品', description: 'キュレーション商品リスト' },
        products: { name: '商品セクション', description: '商品リスト表示' },
        'active-campaigns': { name: '進行中キャンペーン', description: 'アクティブキャンペーンリスト' },
        cta: { name: '下部CTA', description: '会員登録誘導エリア', fixed: true }
      }
    };
    
    const currentLang = language as 'ko' | 'en' | 'jp' || 'ko';
  ...
  const handleDragEnd = async (event: any) => {
    ...
    if (active.id !== over.id) {
      ...
      try {
        ...
        if (response.ok) {
          // JSON 파일 동기화 (모든 언어)
          const syncResponse = await fetch('/api/admin/ui-sections/sync-order', {
            method: 'POST',
            credentials: 'include',
            headers: {
              'Content-Type': 'application/json',
          ...
          if (syncResponse.ok) {
            setSaveMessage({ type: 'success', message: '섹션 순서가 모든 언어에 저장되었습니다.' });
          } else {
            setSaveMessage({ type: 'success', message: '섹션 순서가 저장되었습니다. (JSON 동기화는 수동으로 필요)' });
          }
  ...
  const handleToggleVisibility = async (id: string) => {
    ...
    try {
      ...
      if (response.ok) {
        // JSON 파일 동기화 (섹션 가시성)
        const targetSection = updatedSections.find(s => s.id === id);
        if (targetSection) {
          const syncResponse = await fetch('/api/admin/ui-sections/sync-visibility', {
            method: 'POST',
            credentials: 'include',
            headers: {
              'Content-Type': 'application/json',
          ...
          if (syncResponse.ok) {
            setSaveMessage({ type: 'success', message: '섹션 표시 상태가 모든 언어에 저장되었습니다.' });
          } else {
            setSaveMessage({ type: 'success', message: '섹션 표시 상태가 저장되었습니다. (JSON 동기화는 수동으로 필요)' });
          }
        }
        setTimeout(() => setSaveMessage(null), 3000);
      } else {
  ...
  const handleManualSync = async () => {
    setIsSyncing(true);
    try {
      const response = await fetch('/api/admin/ui-sections/sync-order', {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (response.ok) {
        ...
        setSaveMessage({ 
          type: 'success', 
          message: `JSON 파일이 ${data.updatedLanguages?.join(', ') || '모든 언어'}에 동기화되었습니다.` 
        });
      } else {
        setSaveMessage({ type: 'error', message: 'JSON 동기화에 실패했습니다.' });
      }
    } catch (error) {
      console.error('Manual sync error:', error);
      setSaveMessage({ type: 'error', message: 'JSON 동기화 중 오류가 발생했습니다.' });
    } finally {
      setIsSyncing(false);
      setTimeout(() => setSaveMessage(null), 3000);
    }
        ...
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <p className="text-xs text-blue-600 flex items-center gap-1">
              ...
              변경사항은 자동으로 모든 언어(ko, en, jp)에 동기화됩니다.
            </p>
          </div>
