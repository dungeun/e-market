...
export function LanguageProvider({ children, initialLanguagePacks = {} }: LanguageProviderProps) {
  ...
  // initialLanguagePacks가 변경되면 상태도 업데이트 (하이드레이션 대응)
  useEffect(() => {
    if (Object.keys(initialLanguagePacks).length > 0 && Object.keys(languagePacks).length === 0) {
      setLanguagePacks(initialLanguagePacks);
      setIsLoading(false);
    }
  ...
  // 언어팩 로드
  const loadLanguagePacks = useCallback(async (force = false) => {
    // force가 true가 아니고 이미 언어팩이 있으면 로드하지 않음
    if (!force && Object.keys(languagePacks).length > 0) {
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPackVersion(prev => prev + 1); // Force re-render
      } else {
