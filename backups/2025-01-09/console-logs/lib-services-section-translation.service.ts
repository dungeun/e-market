 ...
 * 슬라이드 번역
 */
async function translateSlide(slide: any, targetLang: string): Promise<any> {
 ...
 * 카테고리 번역
 */
async function translateCategory(category: any, targetLang: string): Promise<any> {
...
export async function saveToLanguagePack(
  ...
  try {
    ...
    console.log(`Language pack saved for section: ${sectionKey}`);
  } catch (error) {
    console.error('Failed to save to language pack:', error);
