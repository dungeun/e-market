...
export class ProductCacheHook {
  ...
  static async onProductCreated(product: Product): Promise<void> {
    console.log(`[Cache Hook] Product created: ${product.id}`);
  ...
  static async onProductUpdated(
    ...
    console.log(`[Cache Hook] Product updated: ${productId}`);
  ...
  static async onProductDeleted(productId: string): Promise<void> {
    console.log(`[Cache Hook] Product deleted: ${productId}`);
  ...
  static async onBulkProductUpdate(productIds: string[]): Promise<void> {
    console.log(`[Cache Hook] Bulk update: ${productIds.length} products`);
  ...
  private static async executeRegeneration(): Promise<void> {
    ...
    console.log(`[Cache Hook] Executing cache regeneration for ${operations.length} operations`);
    ...
    try {
      ...
      console.log('[Cache Hook] Regeneration initiated:', result);
      
    } catch (error) {
      console.error('[Cache Hook] Failed to trigger cache regeneration:', error);
  ...
  private static async triggerAutoTranslation(product: Product): Promise<void> {
    try {
      ...
      if (response.ok) {
        console.log(`[Cache Hook] Auto-translation triggered for product ${product.id}`);
      }
    } catch (error) {
      console.error('[Cache Hook] Auto-translation failed:', error);
    }
  }
  ...
  static async warmCache(): Promise<void> {
    console.log('[Cache Hook] Warming cache...');
    ...
    console.log('[Cache Hook] Cache warming completed');
  }
}
