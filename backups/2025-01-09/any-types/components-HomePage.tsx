...
function HomePage({
  ...
  const [sections] = useState(() => {
    ...
    if (preloadedData?.sectionsData) {
      const sectionsArray = Object.entries(preloadedData.sectionsData).map(([key, section]: [string, any]) => ({
        id: key,
        type: key === 'active-campaigns' ? 'activeCampaigns' : key,
        data: section,
        ...section
      }));
                  ...
                  <h3
                    ...
                    style={{
                      color: (promoData as any).backgroundImage
                        ? "#FFFFFF"
                        : (promoData as any).textColor || "#000000",
                    }}
                  >
  ...
  const renderActiveCampaignsSection = useMemo(() => {
    const activeCampaignsSection = sections.find(
      (s) => s.type === "activeCampaigns" || 
             s.type === "campaigns" || 
             s.type === "active-campaigns" || 
             s.type === "products" || 
             s.type === "active-products"
    );
