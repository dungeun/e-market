...
const CategorySection = React.memo(function CategorySection({ sectionId = 'category', className = '' }: CategorySectionProps) {
  ...
  const loadCategoryData = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/ui-sections/${sectionId}?lang=${currentLanguage}`, {
        headers: {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-4">
            {t('section.category.title', '카테고리')}
          </h2>
          <p className="text-gray-600">
            {t('section.category.description', '원하는 카테고리를 선택하세요')}
          </p>
        </div>
