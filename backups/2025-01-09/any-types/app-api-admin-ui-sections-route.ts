...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await query(`
      INSERT INTO ui_sections (key, type, title, data, "order", "isActive", translations)
      VALUES ($1, $2, $3, $4, $5, $6, $7)
      RETURNING *
    `, [sectionId, type, title, JSON.stringify(content), order || 0, visible !== false, JSON.stringify(translations)]);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const updateResult = await query(`
      ...
      WHERE ${id ? 'id = $' + paramIndex : 'key = $' + paramIndex}
      RETURNING *
    `, updateValues);
