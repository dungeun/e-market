...
const log = {
  ...
  data: (data: any) => console.log(`${colors.yellow}→${colors.reset}`, JSON.stringify(data, null, 2))
}
...
async function runE2ETest() {
  ...
  try {
    ...
    log.data({
      ...
      products: searchResult.products?.map((p: any) => p.name) || []
    })
    ...
    log.data({
      ...
      products: relatedData.relatedProducts.map((p: any) => p.name)
    })
