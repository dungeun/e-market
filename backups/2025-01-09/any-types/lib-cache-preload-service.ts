 ...
 * 홈페이지 데이터를 프리로드
 */
export async function preloadHomePageData(): Promise<any> {
  ...
  try {
    ...
    if (sections && sections.length > 0) {
      ...
      return sections.map(section => ({
        ...
        order: section.order,
        data: section.data as any || {}, // config 대신 data 사용
        config: section.data as any || {} // 이전 호환성 유지
      }));
    }
