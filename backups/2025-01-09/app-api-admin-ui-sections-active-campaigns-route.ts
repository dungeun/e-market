...
// GET: active-campaigns 섹션 가져오기
export async function GET(request: NextRequest) {
  try {
    const result = await query(`
      SELECT * FROM ui_sections 
      WHERE key = 'active-campaigns' OR type = 'active-campaigns'
    ...
    if (result.rows.length === 0) {
      return NextResponse.json({ 
        section: null,
        message: 'Active campaigns section not found'
      });
    }
    ...
    console.error('Error fetching active campaigns section:', error);
    return NextResponse.json({ 
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (autoTranslate && Object.keys(translations).length > 0) {
      await saveToLanguagePack('active-campaigns', { ...translations, ko: content });
    }
    ...
    if (existingResult.rows.length > 0) {
      ...
      const insertResult = await query(`
        ...
        RETURNING *
      `, ['active-campaigns', 'active-campaigns', '진행중인 캠페인', 7, visible, contentJson, translationsJson]);
    ...
    console.error('Error updating active campaigns section:', error);
    return NextResponse.json({ 
