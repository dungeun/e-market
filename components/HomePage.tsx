...
export default function HomePage({ initialSections, initialLanguage = 'ko', initialLanguagePacks = {} }: HomePageProps) {
  ...
  useEffect(() => {
    ...
    if (user && (user.type === 'BUSINESS' || user.type === 'business')) {
      router.push('/business/dashboard')
    }

    // 캠페인 데이터 로드
    loadCampaigns()
  }, [user, router, loadSettingsFromAPI])
  ...
  // 언어별 콘텐츠 가져오기 - 데이터베이스 구조에 맞게 수정
  const getLocalizedContent = (section: any) => {
    const lang = currentLanguage || currentLang
    
    // 데이터베이스에서 가져온 섹션 데이터 구조
    const content = section.data?.content || section.content
    const translations = section.data?.translations || section.translations
    
    if (lang === 'ko' || !translations) {
      return content
    }
