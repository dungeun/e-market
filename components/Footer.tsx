...
const Footer = React.memo(function Footer() {
  ...
  const footerEnabled = siteSettings.footer?.enabled ?? siteSettings.website?.footerEnabled ?? websiteSettings?.footerEnabled ?? true
        ...
        <div className="grid md:grid-cols-4 gap-8">
          ...
          <div className="col-span-1">
            <h3 className="text-2xl font-bold bg-gradient-to-r from-cyan-400 to-emerald-400 bg-clip-text text-transparent mb-4">
              {siteSettings.footer?.companyName || siteSettings.general?.siteName || config.header.logo.text}
            </h3>
            <p className="text-gray-600 dark:text-gray-400 mb-4">
              {siteSettings.general?.description || '브랜드와 인플루언서를 연결하는 스마트한 마케팅 플랫폼'}
            </p>
            <div className="flex space-x-4">
              {/* 관리자 설정의 소셜 링크 우선 사용 */}
              {siteSettings.footer?.socialLinks ? (
                Object.entries(siteSettings.footer.socialLinks)
                  .filter(([_, url]) => url)
                  .map(([platform, url]) => {
          ...
          {siteSettings.footer?.footerLinks && siteSettings.footer.footerLinks.length > 0 ? (
            <div className="col-span-3 grid md:grid-cols-3 gap-8">
              <div>
                <h4 className="text-lg font-semibold mb-4">서비스</h4>
                <ul className="space-y-2">
                  {siteSettings.footer.footerLinks.map((link, index) => (
                    <li key={index}>
                      <Link 
                        href={link.url} 
                        className="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors"
        ...
        <div className="border-t border-gray-300 dark:border-gray-800 mt-8 pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="text-gray-600 dark:text-gray-400 text-sm mb-4 md:mb-0">
              <p>{siteSettings.footer?.copyrightText || siteSettings.website?.footerText || t(copyright, copyright)}</p>
              <p className="mt-1">
                사업자등록번호: {siteSettings.footer?.businessNumber || '123-45-67890'} | 
                대표: {siteSettings.footer?.ceoName || '홍길동'} | 
                <br className="md:hidden" />
                주소: {siteSettings.footer?.address || '서울특별시 강남구 테헤란로 123, 456호'}
              </p>
            </div>
            
            <div className="flex items-center space-x-4">
              <div className="text-gray-600 dark:text-gray-400 text-sm">
                <p>고객센터: {siteSettings.footer?.customerServicePhone || siteSettings.general?.supportEmail || '1588-1234'}</p>
                <p>{siteSettings.footer?.operatingHours || '평일 09:00~18:00 (주말/공휴일 휴무)'}</p>
              </div>
            </div>
          </div>
        </div>
