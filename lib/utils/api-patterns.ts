...
interface ApiHandlerOptions {
  ...
  cache?: {
    ttl: number;
    keyBuilder?: (req: NextRequest, params?: unknown) => string;
  };
  validation?: {
    body?: z.ZodSchema;
    query?: z.ZodSchema;
...
// 기본 캐시 키 생성
function generateDefaultCacheKey(req: NextRequest, params?: unknown): string {
...
export const ApiPatterns = {
   ...
   * CRUD 리스트 조회 패턴
   */
  list: (handler: (req: NextRequest, context: ApiContext) => Promise<unknown>) =>
    createApiHandler(
      async (req, context) => {
   ...
   */
  protectedGet: (
    handler: (req: NextRequest, context: ApiContext) => Promise<unknown>,
    roles?: string[]
  ) =>
    createApiHandler(
      async (req, context) => {
