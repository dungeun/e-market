...
class Logger {
  ...
  public error(message: string, error?: Error | any, data?: unknown): void {
  ...
  // API 요청/응답 로깅
  public api(req: unknown, res: unknown, duration: number): void {
...
export const loggerMiddleware = (req: unknown, res: unknown, next: unknown) => {
  const start = Date.now();
