...
export const cacheMetadata = ${JSON.stringify(metadata, null, 2)};`,
   ...
   * 캠페인 데이터 변환
   */
  private transformCampaign(campaign: unknown): any {
    return {
      id: campaign.id,
      ...
      category:
        campaign.categories.find((c: unknown) => c.isPrimary)?.category || null,
      stats: {
   ...
   * 카테고리별 그룹화
   */
  private groupByCategory(campaigns: unknown[]): Record<string, any[]> {
    const grouped: Record<string, any[]> = {};

    campaigns.forEach((campaign) => {
      const primaryCategory = campaign.categories.find((c: unknown) => c.isPrimary);
      if (primaryCategory) {
   ...
   * 플랫폼별 그룹화
   */
  private groupByPlatform(campaigns: unknown[]): Record<string, any[]> {
    const grouped: Record<string, any[]> = {};
  ...
  async incrementalUpdate(newCampaignId: string): Promise<void> {
    try {
      // 기존 캐시 읽기
      const existing = await this.readCache<unknown>("campaigns.json");
      if (!existing) {
        // 캐시가 없으면 전체 재생성
