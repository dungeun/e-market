...
export class EnhancedCacheService {
   ...
   * 캠페인 목록 캐싱
   */
  async getCampaigns(params: unknown, fetcher: () => Promise<unknown>): Promise<unknown> {
   ...
   * UI 설정 캐싱
   */
  async getUIConfig(lang: string, fetcher: () => Promise<unknown>): Promise<unknown> {
   ...
   * 통계 데이터 캐싱
   */
  async getStats(type: string, fetcher: () => Promise<unknown>): Promise<unknown> {
