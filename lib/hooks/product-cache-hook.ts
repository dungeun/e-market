...
export class ProductCacheHook {
  ...
  private static async executeRegeneration(): Promise<void> {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_APP_URL || env.appUrl}/api/admin/regenerate-cache`,
        {
          method: 'POST',
  ...
  private static async triggerAutoTranslation(product: Product): Promise<void> {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_APP_URL || env.appUrl}/api/admin/products/${product.id}/translate`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
...
export function registerProductCacheHooks(prisma: unknown): void {
  // Prisma 미들웨어 사용 예시
  prisma.$use(async (params: unknown, next: unknown) => {
