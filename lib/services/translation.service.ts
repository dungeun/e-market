...
export class TranslationService {
   ...
   * 캠페인 데이터 번역
   */
  async translateCampaignData(campaignData: unknown): Promise<unknown> {
    const fieldsToTranslate = ['title', 'description', 'requirements'];
    const translatedData: unknown = {};
   ...
   * 메뉴 아이템 번역
   */
  async translateMenuItem(menuItem: unknown, targetLang: string = 'en'): Promise<unknown> {
    if (!menuItem.label) {
      return menuItem;
    }
