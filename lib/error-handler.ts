...
// Prisma types removed - using SQL types;
...
function filterErrorMessage(error: Error | unknown): string {
...
function determineErrorType(error: Error | unknown): ErrorType {
  if (error instanceof AppError) {
    return error.type;
  }
...
function determineStatusCode(error: Error | unknown): number {
  if (error instanceof AppError) {
...
function getErrorDetails(error: Error | unknown): any {
  if (config.isProduction) {
    return undefined;
  }
