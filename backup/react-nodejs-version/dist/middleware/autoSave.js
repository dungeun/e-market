...
exports.shutdownAutoSave = exports.getAutoSaveStats = exports.cleanupAutoSave = exports.recoverCartFromSnapshot = exports.autoSaveMiddleware = void 0;
...
function autoSaveMiddleware(config = {}) {
    ...
    return async (req, res, next) => {
        try {
            ...
            res.json = function (data) {
                ...
                setImmediate(async () => {
                    try {
                        ...
                        logger_1.logger.error('Error in auto-save after cart operation:', error);
                    }
                });
                return result;
            };
            ...
            logger_1.logger.error('Auto-save middleware error:', error);
 ...
 */
async function setupAutoSave(sessionId, config) {
    ...
    const timer = setInterval(async () => {
        try {
            ...
            logger_1.logger.error(`Periodic auto-save error for session ${sessionId}:`, error);
        }
    }, config.interval * 1000);
    saveTimers.set(sessionId, timer);
    logger_1.logger.debug(`Auto-save setup for session: ${sessionId}, interval: ${config.interval}s`);
}
/**
 * Perform auto-save for a specific cart
 */
async function performAutoSave(sessionId, cartId, config, retryCount = 0) {
    try {
        const cart = await cartService_1.cartService.getCartById(cartId);
        if (!cart) {
            logger_1.logger.warn(`Cart not found for auto-save: ${cartId}`);
            return;
        }
        ...
        if (config.onlyOnChanges) {
            ...
            if (previousSnapshot && previousSnapshot.checksum === snapshot.checksum) {
                logger_1.logger.debug(`No changes detected for cart ${cartId}, skipping auto-save`);
                return;
            }
        }
        // Store snapshot
        ...
        logger_1.logger.debug(`Auto-save completed for cart ${cartId}`, {
            sessionId,
            itemCount: cart.items.length,
        ...
        logger_1.logger.error(`Auto-save failed for cart ${cartId}:`, error);
        // Retry logic
        if (retryCount < config.maxRetries) {
            logger_1.logger.info(`Retrying auto-save for cart ${cartId}, attempt ${retryCount + 1}`);
            setTimeout(() => {
                performAutoSave(sessionId, cartId, config, retryCount + 1);
            }, Math.pow(2, retryCount) * 1000); // Exponential backoff
        }
        else {
            logger_1.logger.error(`Auto-save failed after ${config.maxRetries} attempts for cart ${cartId}`);
        }
    }
}
/**
 ...
 */
async function performPeriodicSave(sessionId, config) {
    try {
        ...
        logger_1.logger.error(`Periodic save error for session ${sessionId}:`, error);
    }
}
/**
 * Recover cart state from snapshot
 */
async function recoverCartFromSnapshot(sessionId) {
    try {
        const snapshot = cartSnapshots.get(sessionId);
        if (!snapshot) {
            logger_1.logger.debug(`No snapshot found for session ${sessionId}`);
            return false;
        }
        // Check if snapshot is recent (within last hour)
        const hourAgo = new Date(Date.now() - 60 * 60 * 1000);
        if (snapshot.timestamp < hourAgo) {
            logger_1.logger.debug(`Snapshot too old for session ${sessionId}`);
        ...
        if (cart.items.length > 0) {
            await cartService_1.cartService.clearCart(cart.id);
        }
        // Restore items from snapshot
        for (const item of snapshot.items) {
            try {
                await cartService_1.cartService.addItemToCart(cart.id, {
                ...
                logger_1.logger.warn('Failed to restore cart item during recovery:', error);
                // Continue with other items
            }
        }
        logger_1.logger.info(`Cart recovered from snapshot for session ${sessionId}`, {
        ...
        logger_1.logger.error(`Cart recovery failed for session ${sessionId}:`, error);
        return false;
    }
}
exports.recoverCartFromSnapshot = recoverCartFromSnapshot;
/**
 * Clean up auto-save resources for a session
 */
function cleanupAutoSave(sessionId) {
    // Clear timer
    const timer = saveTimers.get(sessionId);
    if (timer) {
        clearInterval(timer);
    ...
    // Clear snapshot
    cartSnapshots.delete(sessionId);
    logger_1.logger.debug(`Auto-save cleanup completed for session: ${sessionId}`);
}
exports.cleanupAutoSave = cleanupAutoSave;
/**
...
exports.getAutoSaveStats = getAutoSaveStats;
// Helper functions
function isCartOperation(req) {
...
function createCartSnapshot(cart) {
    ...
    const coupons = cart.appliedCoupons?.map((coupon) => coupon.code) || [];
...
function shutdownAutoSave() {
    logger_1.logger.info('Shutting down auto-save system...');
    // Clear all timers
    ...
    // Clear all snapshots
    cartSnapshots.clear();
    logger_1.logger.info('Auto-save system shutdown complete');
}
exports.shutdownAutoSave = shutdownAutoSave;
