...
const productOptionsService_1 = require("./productOptionsService");
class CartService {
    constructor() {
        this.CART_EXPIRY_HOURS = 72; // 3 days
        this.MAX_CART_ITEMS = 100;
        this.MIN_QUANTITY = 1;
    ...
    async createCart(data) {
        try {
            ...
            const cart = await database_1.prisma.$transaction(async (tx) => {
                // Create the cart
            ...
            logger_1.logger.info(`Cart created: ${cart.id}`);
    ...
    async getCartByUserOrSession(userId, sessionId) {
        if (!userId && !sessionId) {
            throw new error_1.AppError('Either userId or sessionId must be provided', 400);
        }
        const where = {
            OR: [],
            expiresAt: {
    ...
    async updateCart(id, data) {
        ...
        logger_1.logger.info(`Cart updated: ${id}`);
        return this.getCartWithDetails(id);
    }
    // Delete cart
    async deleteCart(id) {
    ...
    async addItemToCart(cartId, itemData) {
        await database_1.prisma.$transaction(async (transaction) => {
            await this.performAddItemToCart(transaction, cartId, itemData);
        });
        logger_1.logger.info(`Item added to cart ${cartId}: ${itemData.productId}`);
        return this.getCartWithDetails(cartId);
    }
    async performAddItemToCart(tx, cartId, data) {
        ...
        if (cart.items.length >= this.MAX_CART_ITEMS) {
            throw new error_1.AppError(`Cannot add more than ${this.MAX_CART_ITEMS} items to cart`, 400);
        }
        ...
        if (data.variantId) {
            ...
            if (!variant || !variant.isActive) {
                throw new error_1.AppError('Product variant not found or inactive', 404);
            }
        }
        ...
        if (product.trackQuantity || (variant && product.trackQuantity)) {
            if (availableQuantity < data.quantity) {
                throw new error_1.AppError('Insufficient stock available', 400);
            }
        }
        // Validate product options if provided and calculate final price
        let finalPrice = variant ? Number(variant.price) : Number(product.price);
        if (data.options) {
            try {
                // Validate options
                const validation = await productOptionsService_1.productOptionsService.validateProductOptions(data.productId, data.options);
                if (!validation.isValid) {
                    const errorMessages = validation.errors.map(e => e.message).join(', ');
                    throw new error_1.AppError(`Invalid product options: ${errorMessages}`, 400);
                }
                // Calculate price with options
                const pricing = await productOptionsService_1.productOptionsService.calculateOptionPricing(data.productId, data.options);
                ...
                logger_1.logger.warn('Error validating product options:', error);
        ...
        if (existingItem) {
            ...
            if (newQuantity > this.MAX_QUANTITY) {
                throw new error_1.AppError(`Quantity cannot exceed ${this.MAX_QUANTITY}`, 400);
            }
            // Check stock for updated quantity
            if (product.trackQuantity && availableQuantity < newQuantity) {
                throw new error_1.AppError('Insufficient stock for requested quantity', 400);
            }
            await tx.cartItem.update({
                where: { id: existingItem.id },
                data: {
                    quantity: newQuantity,
                    price: new client_1.Prisma.Decimal(finalPrice), // Update price in case it changed
                    updatedAt: new Date(),
                },
            });
        }
            ...
            await tx.cartItem.create({
                data: {
                    ...
                    price: new client_1.Prisma.Decimal(finalPrice),
    ...
    async updateCartItem(cartId, itemId, data) {
        ...
        if (!cartItem) {
            throw new error_1.AppError('Cart item not found', 404);
        }
        // Validate quantity
        if (data.quantity < this.MIN_QUANTITY || data.quantity > this.MAX_QUANTITY) {
            throw new error_1.AppError(`Quantity must be between ${this.MIN_QUANTITY} and ${this.MAX_QUANTITY}`, 400);
        }
        // Check stock availability
        ...
        let finalPrice = cartItem.variant ? Number(cartItem.variant.price) : Number(cartItem.product.price);
        if (data.options) {
            try {
                const validation = await productOptionsService_1.productOptionsService.validateProductOptions(cartItem.productId, data.options);
                if (!validation.isValid) {
                    const errorMessages = validation.errors.map(e => e.message).join(', ');
                    throw new error_1.AppError(`Invalid product options: ${errorMessages}`, 400);
                }
                const pricing = await productOptionsService_1.productOptionsService.calculateOptionPricing(cartItem.productId, data.options);
                finalPrice = Number(pricing.finalPrice);
            }
            catch (error) {
                if (error instanceof error_1.AppError) {
                ...
                logger_1.logger.warn('Error validating product options during update:', error);
                // Continue without options validation if service is unavailable
            }
        }
        await database_1.prisma.$transaction(async (tx) => {
            ...
            if (data.options) {
                updateData.price = new client_1.Prisma.Decimal(finalPrice);
        ...
        logger_1.logger.info(`Cart item updated: ${itemId} in cart ${cartId}`);
        return this.getCartWithDetails(cartId);
    }
    ...
    async removeCartItem(cartId, itemId) {
        ...
        logger_1.logger.info(`Cart item removed: ${itemId} from cart ${cartId}`);
        return this.getCartWithDetails(cartId);
    }
    ...
    async applyCoupon(cartId, data) {
        ...
        if (!coupon) {
            throw new error_1.AppError('Invalid coupon code', 400);
        }
        // Validate coupon
        if (!coupon.isActive) {
            throw new error_1.AppError('Coupon is inactive', 400);
        }
        if (coupon.expiresAt && coupon.expiresAt < new Date()) {
            throw new error_1.AppError('Coupon has expired', 400);
        }
        if (coupon.startsAt && coupon.startsAt > new Date()) {
            throw new error_1.AppError('Coupon is not yet active', 400);
        }
        if (coupon.usageLimit && coupon.usageCount >= coupon.usageLimit) {
            throw new error_1.AppError('Coupon usage limit exceeded', 400);
        }
        // Check if coupon is already applied
        const existingCoupon = cart.coupons.find(cc => cc.coupon.code === data.couponCode);
        if (existingCoupon) {
            throw new error_1.AppError('Coupon is already applied to this cart', 400);
        }
        ...
        logger_1.logger.info(`Coupon applied to cart ${cartId}: ${data.couponCode}`);
        return this.getCartWithDetails(cartId);
    }
    ...
    async removeCoupon(cartId, couponId) {
        ...
        logger_1.logger.info(`Coupon removed from cart ${cartId}: ${couponId}`);
        return this.getCartWithDetails(cartId);
    }
    // Merge two carts
    async mergeCarts(data) {
        ...
        if (!sourceCart) {
            throw new error_1.AppError('Source cart not found', 404);
        }
        if (!targetCart) {
            throw new error_1.AppError('Target cart not found', 404);
        }
        if (this.isCartExpired(sourceCart) || this.isCartExpired(targetCart)) {
            throw new error_1.AppError('One or both carts have expired', 410);
        }
        await database_1.prisma.$transaction(async (tx) => {
        ...
        logger_1.logger.info(`Carts merged: ${sourceCartId} -> ${targetCartId}`);
    ...
    async transferCart(data) {
        ...
        if (userCart) {
            ...
            logger_1.logger.info(`Cart transferred from session ${sessionId} to user ${userId}`);
    ...
    async cleanupExpiredCarts() {
        ...
        logger_1.logger.info(`Cleaned up ${result.count} expired carts`);
        return result.count;
    }
