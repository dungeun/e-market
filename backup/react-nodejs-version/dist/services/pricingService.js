...
class PricingService {
    ...
    async calculateBulkPricing(request) {
        try {
            const results = [];
    ...
    async getApplicablePricingRules(request, product) {
        const now = new Date();
        ...
        const uniqueRules = allRules.filter((rule, index, self) => index === self.findIndex(r => r.id === rule.id));
    ...
    async evaluateRuleConditions(rule, request, product) {
        try {
            ...
            if (rule.type === 'QUANTITY_DISCOUNT') {
                ...
                if (request.quantity < minQuantity)
                    return false;
                if (maxQuantity && request.quantity > maxQuantity)
                    return false;
            }
            ...
            if (rule.type === 'INVENTORY_BASED') {
                ...
                if (conditions.stockRange) {
                    ...
                    if (min !== undefined && product.quantity < min)
                        return false;
                    if (max !== undefined && product.quantity > max)
                        return false;
                }
            }
            // Cart value conditions
            if (conditions.minCartValue && request.context?.cartTotal) {
                if (request.context.cartTotal < conditions.minCartValue)
                    return false;
            }
            if (conditions.maxCartValue && request.context?.cartTotal) {
                if (request.context.cartTotal > conditions.maxCartValue)
                    return false;
            }
            ...
            logger_1.logger.error('Error evaluating rule conditions:', error);
            return false;
        }
    }
    async applyPricingRule(rule, context) {
        try {
            ...
            logger_1.logger.error('Error applying pricing rule:', error);
        ...
        try {
            await database_1.prisma.pricingApplication.create({
                data: {
                    ...
                    originalPrice: new library_1.Decimal(data.originalPrice),
                    discountAmount: new library_1.Decimal(data.discountAmount),
                    finalPrice: new library_1.Decimal(data.finalPrice),
            ...
            logger_1.logger.error('Error recording pricing application:', error);
        }
    }
        ...
        try {
            const rule = await database_1.prisma.pricingRule.create({
                data: {
                    ...
                    maxDiscountValue: data.maxDiscountValue ? new library_1.Decimal(data.maxDiscountValue) : null,
        ...
        try {
            const rule = await database_1.prisma.pricingRule.update({
                ...
                data: {
                    ...
                    conditions: data.conditions,
                    discountType: data.discountType,
                    discountValue: data.discountValue ? new library_1.Decimal(data.discountValue) : undefined,
                    maxDiscountValue: data.maxDiscountValue ? new library_1.Decimal(data.maxDiscountValue) : undefined,
