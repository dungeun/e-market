...
class OrderService {
    constructor() {
        this.ORDER_NUMBER_PREFIX = 'ORD';
        this.MAX_REFUND_DAYS = 30;
    }
    ...
    async createOrderFromCart(data) {
        try {
            ...
            const cart = await cartService_1.cartService.getCartById(data.cartId);
            if (!cart || cart.items.length === 0) {
                throw new error_1.AppError('Cart is empty or not found', 400);
            }
            // Validate cart stock
            const stockValidation = await cartService_1.cartService.validateCartStock(data.cartId);
            if (!stockValidation.isValid) {
                throw new error_1.AppError('Some items in cart have insufficient stock', 400);
            }
            // Get addresses
            ...
            if (!shippingAddress) {
                throw new error_1.AppError('Shipping address not found', 404);
            }
            const order = await database_1.prisma.$transaction(async (tx) => {
                ...
                if (cart.appliedCoupons.length > 0) {
                    await Promise.all(cart.appliedCoupons.map(async (coupon) => {
            ...
            logger_1.logger.info(`Order created: ${order.orderNumber} from cart: ${data.cartId}`);
            return this.getOrderWithDetails(order.id);
        }
        catch (error) {
            logger_1.logger.error('Error creating order from cart:', error);
            throw error;
        }
    }
    ...
    async updateOrder(id, data) {
        ...
        logger_1.logger.info(`Order updated: ${id}`);
        return this.getOrderWithDetails(id);
    }
    // Cancel order
    async cancelOrder(id, data) {
        ...
        if (!['PENDING', 'CONFIRMED', 'PROCESSING'].includes(order.status)) {
            throw new error_1.AppError('Order cannot be cancelled in current status', 400);
        }
        await database_1.prisma.$transaction(async (tx) => {
            // Update order status
            await tx.order.update({
        ...
        logger_1.logger.info(`Order cancelled: ${id}, reason: ${data.reason}`);
        return this.getOrderWithDetails(id);
    }
    // Process refund
    async processRefund(id, data) {
        ...
        if (!['DELIVERED', 'COMPLETED'].includes(order.status)) {
            throw new error_1.AppError('Order cannot be refunded in current status', 400);
        }
        // Check refund period
        const daysSinceDelivery = Math.floor((Date.now() - new Date(order.updatedAt).getTime()) / (1000 * 60 * 60 * 24));
        if (daysSinceDelivery > this.MAX_REFUND_DAYS) {
            throw new error_1.AppError(`Refund period of ${this.MAX_REFUND_DAYS} days has expired`, 400);
        }
        ...
        if (data.amount > maxRefundAmount) {
            throw new error_1.AppError(`Refund amount exceeds maximum refundable amount of ${maxRefundAmount}`, 400);
        }
        await database_1.prisma.$transaction(async (tx) => {
        ...
        logger_1.logger.info(`Order refund processed: ${id}, amount: ${data.amount}`);
        return this.getOrderWithDetails(id);
    }
    ...
    async updateShipping(id, data) {
        ...
        logger_1.logger.info(`Order shipping updated: ${id}`);
        return this.getOrderWithDetails(id);
    }
    ...
    async markAsDelivered(id) {
        ...
        if (order.status !== 'SHIPPED') {
            throw new error_1.AppError('Only shipped orders can be marked as delivered', 400);
        }
        await database_1.prisma.$transaction(async (tx) => {
            await tx.order.update({
                where: { id },
        ...
        logger_1.logger.info(`Order marked as delivered: ${id}`);
        return this.getOrderWithDetails(id);
    }
    // Get order timeline
    ...
    async getOrderAnalytics(userId) {
        ...
        const statusCountsFormatted = statusCounts.map(sc => ({
            status: sc.status,
    ...
    validateStatusTransition(currentStatus, newStatus) {
        ...
        if (!validTransitions[currentStatus].includes(newStatus)) {
            throw new error_1.AppError(`Invalid status transition from ${currentStatus} to ${newStatus}`, 400);
        }
    }
    async createTimelineEvent(tx, orderId, event) {
