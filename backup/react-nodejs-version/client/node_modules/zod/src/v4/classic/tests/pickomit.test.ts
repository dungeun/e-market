...
test("pick parse - fail", () => {
  fish.pick({ name: true }).parse({ name: "12" } as unknown);
  fish.pick({ name: true }).parse({ name: "bob", age: 12 } as unknown);
  fish.pick({ age: true }).parse({ age: 12 } as unknown);

  const nameonlyFish = fish.pick({ name: true }).strict();
  const bad1 = () => nameonlyFish.parse({ name: 12 } as unknown);
  const bad2 = () => nameonlyFish.parse({ name: "bob", age: 12 } as unknown);
  const bad3 = () => nameonlyFish.parse({ age: 12 } as unknown);

  // @ts-expect-error checking runtime picks `name` only.
  const anotherNameonlyFish = fish.pick({ name: true, age: false }).strict();
  const bad4 = () => anotherNameonlyFish.parse({ name: "bob", age: 12 } as unknown);
...
test("omit parse - fail", () => {
  const nonameFish = fish.omit({ name: true });
  const bad1 = () => nonameFish.parse({ name: 12 } as unknown);
  const bad2 = () => nonameFish.parse({ age: 12 } as unknown);
  const bad3 = () => nonameFish.parse({} as unknown);

  // @ts-expect-error checking runtime omits `name` only.
  const anotherNonameFish = fish.omit({ name: true, age: false });
  const bad4 = () => anotherNonameFish.parse({ nested: {} } as unknown);
...
test("nonstrict parsing - fail", () => {
  const laxfish = fish.passthrough().pick({ name: true });
  const bad = () => laxfish.parse({ whatever: "asdf" } as unknown);
  expect(bad).toThrow();
});
