...
test("mutual recursion with cyclical data", () => {
  const a: unknown = { val: 1 };
  const b: unknown = { val: 2 };
  a.b = b;
  b.a = a;
});
...
test("lazy initialization", () => {
  const a: unknown = z.lazy(() => a).optional();
  const b: unknown = z.lazy(() => b).nullable();
  const c: unknown = z.lazy(() => c).default({} as unknown);
  const d: unknown = z.lazy(() => d).prefault({} as unknown);
  const e: unknown = z.lazy(() => e).nonoptional();
  const f: unknown = z.lazy(() => f).catch({} as unknown);
  const g: unknown = z.lazy(() => z.object({ g })).readonly();
