...
export interface _$ZodTypeInternals {
  ...
  run(payload: ParsePayload<unknown>, ctx: ParseContextInternal): util.MaybeAsync<ParsePayload>;

  /** @internal Parses input, doesn't run checks. */
  parse(payload: ParsePayload<unknown>, ctx: ParseContextInternal): util.MaybeAsync<ParsePayload>;
...
export const $ZodType: core.$constructor<$ZodType> = /*@__PURE__*/ core.$constructor("$ZodType", (inst, def) => {
  ...
  if (inst._zod.traits.has("$ZodCheck")) {
    checks.unshift(inst as unknown);
  }
  //
  ...
  if (checks.length === 0) {
    ...
    const runChecks = (
      ...
      for (const ch of checks) {
        ...
        const _ = ch._zod.check(payload as unknown) as unknown as ParsePayload;
...
function handleArrayResult(result: ParsePayload<unknown>, final: ParsePayload<any[]>, index: number) {
...
function handleObjectResult(result: ParsePayload, final: ParsePayload, key: PropertyKey) {
  ...
  (final.value as unknown)[key] = result.value;
}

function handleOptionalObjectResult(result: ParsePayload, final: ParsePayload, key: PropertyKey, input: unknown) {
  if (result.issues.length) {
    // validation failed against value schema
    if (input[key] === undefined) {
      // if input was undefined, ignore the error
      if (key in input) {
        (final.value as unknown)[key] = undefined;
      } else {
        (final.value as unknown)[key] = result.value;
      }
    } else {
    ...
    if (key in input) (final.value as unknown)[key] = undefined;
  } else {
    // non-undefined value
    (final.value as unknown)[key] = result.value;
  }
...
export type $ZodLooseShape = Record<string, unknown>;
...
export const $ZodObject: core.$constructor<$ZodObject> = /*@__PURE__*/ core.$constructor("$ZodObject", (inst, def) => {
  ...
  const generateFastpass = (shape: unknown) => {
    ...
    const ids: unknown = Object.create(null);
    let counter = 0;
    ...
    return (payload: unknown, ctx: unknown) => fn(shape, payload, ctx);
  };
...
export const $ZodDiscriminatedUnion: core.$constructor<$ZodDiscriminatedUnion> =
  ...
  core.$constructor("$ZodDiscriminatedUnion", (inst, def) => {
    ...
    inst._zod.parse = (payload, ctx) => {
      ...
      const opt = disc.value.get(input?.[def.discriminator] as unknown);
      if (opt) {
        return opt._zod.run(payload, ctx) as unknown;
      }
...
function handleSetResult(result: ParsePayload, final: ParsePayload<Set<unknown>>) {
  if (result.issues.length) {
...
//     inst._zod.values = new Set<util.Primitive>(def.value as unknown);
//   }

//   if (util.propertyKeyTypes.has(typeof def.value)) {
//     inst._zod.pattern = new RegExp(
//       `^(${typeof def.value === "string" ? util.escapeRegex(def.value) : (def.value as unknown).toString()})$`
//     );
//   } else {
    ...
    for (const part of def.parts) {
      if (part instanceof $ZodType) {
        if (!part._zod.pattern) {
          // if (!source)
          throw new Error(`Invalid template literal part, no pattern found: ${[...(part as unknown)._zod.traits].shift()}`);
        }
...
export const $ZodCustom: core.$constructor<$ZodCustom> = /*@__PURE__*/ core.$constructor("$ZodCustom", (inst, def) => {
  ...
  inst._zod.check = (payload) => {
    const input = payload.value;
    const r = def.fn(input as unknown);
    if (r instanceof Promise) {
