...
export function jsonStringifyReplacer(_: string, value: unknown): any {
...
export function nullish(input: unknown): boolean {
...
export function getElementAtPath(obj: unknown, path: (string | number)[] | null | undefined): any {
  if (!path) return obj;
...
export function promiseAllObject<T extends object>(promisesObj: T): Promise<{ [k in keyof T]: Awaited<T[k]> }> {
  ...
  const promises = keys.map((key) => (promisesObj as unknown)[key]);

  return Promise.all(promises).then((results) => {
    const resolvedObj: unknown = {};
...
export function numKeys(data: unknown): number {
  let keyCount = 0;
...
export const getParsedType = (data: unknown): ParsedTypes => {
  const t = typeof data;
...
export function normalizeParams<T>(_params: T): Normalize<T> {
  const params: unknown = _params;

  if (!params) return {} as unknown;
  if (typeof params === "string") return { error: () => params } as unknown;
  if (params?.message !== undefined) {
    if (params?.error !== undefined) throw new Error("Cannot specify both `message` and `error` params");
    params.error = params.message;
  }
  delete params.message;
  if (typeof params.error === "string") return { ...params, error: () => params.error } as unknown;
  return params;
}
...
export function omit(schema: schemas.$ZodObject, mask: object): any {
  ...
  for (const key in mask) {
    ...
    if (!(mask as unknown)[key]) continue;
...
export function extend(schema: schemas.$ZodObject, shape: schemas.$ZodShape): any {
  ...
  return clone(schema, def) as unknown;
}
...
export function prefixIssues(path: PropertyKey, issues: errors.$ZodRawIssue[]): errors.$ZodRawIssue[] {
  return issues.map((iss) => {
    (iss as unknown).path ??= [];
    (iss as unknown).path.unshift(path);
    return iss;
  });
}
...
export function finalizeIssue(
  ...
  if (!iss.message) {
    const message =
      ...
      "Invalid input";
    (full as unknown).message = message;
  }

  // delete (full as unknown).def;
  delete (full as unknown).inst;
  delete (full as unknown).continue;
  if (!ctx?.reportInput) {
    delete (full as unknown).input;
  }

  return full;
}

export function getSizableOrigin(input: unknown): "set" | "map" | "file" | "unknown" {
...
export function getLengthableOrigin(input: unknown): "array" | "string" | "unknown" {
...
//////////    REFINES     //////////
export function issue(_iss: string, input: unknown, inst: unknown): errors.$ZodRawIssue;
