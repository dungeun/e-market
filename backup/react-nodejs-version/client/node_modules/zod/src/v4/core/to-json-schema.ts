...
interface JSONSchemaGeneratorParams {
  ...
  metadata?: $ZodRegistry<Record<string, unknown>>;
...
export class JSONSchemaGenerator {
  metadataRegistry: $ZodRegistry<Record<string, unknown>>;
  target: "draft-7" | "draft-2020-12";
  unrepresentable: "throw" | "any";
  override: (ctx: {
    zodSchema: schemas.$ZodTypes;
  ...
  process(schema: schemas.$ZodType, _params: ProcessParams = { path: [], schemaPath: [] }): JSONSchema.BaseSchema {
    ...
    if (overrideSchema) {
      result.schema = overrideSchema as unknown;
    } else {
      const params = {
        ..._params,
      ...
      if (parent) {
        ...
        switch (def.type) {
          case "string": {
            const json: JSONSchema.StringSchema = _json as unknown;
            json.type = "string";
          ...
          case "number": {
            const json: JSONSchema.NumberSchema | JSONSchema.IntegerSchema = _json as unknown;
          ...
          case "array": {
            const json: JSONSchema.ArraySchema = _json as unknown;
          ...
          case "object": {
            const json: JSONSchema.ObjectSchema = _json as unknown;
            json.type = "object";
            json.properties = {};
          ...
          case "union": {
            const json: JSONSchema.BaseSchema = _json as unknown;
          ...
          case "intersection": {
            ...
            const isSimpleIntersection = (val: unknown) => "allOf" in val && Object.keys(val).length === 1;
            const allOf = [
              ...(isSimpleIntersection(a) ? (a.allOf as unknown[]) : [a]),
              ...(isSimpleIntersection(b) ? (b.allOf as unknown[]) : [b]),
            ];
            json.allOf = allOf;
            break;
          }
          ...
          case "literal": {
            ...
            if (vals.length === 0) {
              ...
              const val = vals[0]!;
              json.type = val === null ? ("null" as const) : (typeof val as unknown);
              json.const = val;
            } else {
          ...
          case "catch": {
            ...
            try {
              catchValue = def.catchValue(undefined as unknown);
            } catch {
...
function isTransforming(
  ...
  throw new Error(`Unknown schema type: ${(def as unknown).type}`);
}
