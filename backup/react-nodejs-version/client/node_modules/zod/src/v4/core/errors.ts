...
export function formatError<T>(error: $ZodError, _mapper?: unknown) {
  const mapper: (issue: $ZodIssue) => any =
    _mapper ||
    function (issue: $ZodIssue) {
      return issue.message;
    };
  const fieldErrors: $ZodFormattedError<T> = { _errors: [] } as unknown;
  const processError = (error: { issues: $ZodIssue[] }) => {
    for (const issue of error.issues) {
      if (issue.code === "invalid_union" && issue.errors.length) {
        ...
        (fieldErrors as unknown)._errors.push(mapper(issue));
      } else {
        let curr: unknown = fieldErrors;
        let i = 0;
...
export function treeifyError<T>(error: $ZodError, _mapper?: unknown) {
  const mapper: (issue: $ZodIssue) => any =
    _mapper ||
    function (issue: $ZodIssue) {
      return issue.message;
    };
  const result: $ZodErrorTree<T> = { errors: [] } as unknown;
