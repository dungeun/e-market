...
class ParseInputLazyPath implements ParseInput {
  ...
  constructor(parent: ParseContext, value: unknown, path: ParsePath, key: string | number | (string | number)[]) {
    this.parent = parent;
    this.data = value;
    this._path = path;
    this._key = key;
...
const handleResult = <Input, Output>(
  ...
  if (isValid(result)) {
    ...
    return {
      success: false,
      get error() {
        if ((this as unknown)._error) return (this as unknown)._error as Error;
        const error = new ZodError(ctx.common.issues);
        (this as unknown)._error = error;
        return (this as unknown)._error;
      },
    };
  }
};
...
export abstract class ZodType<Output = any, Def extends ZodTypeDef = ZodTypeDef, Input = Output> {
  ...
  "~validate"(data: unknown): StandardSchemaV1.Result<Output> | Promise<StandardSchemaV1.Result<Output>> {
    const ctx: ParseContext = {
      common: {
        issues: [],
        async: !!(this["~standard"] as unknown).async,
      },
      path: [],
      schemaErrorMap: this._def.errorMap,
      parent: null,
    ...
    if (!(this["~standard"] as unknown).async) {
      try {
        ...
        if ((err as Error)?.message?.toLowerCase()?.includes("encountered")) {
          (this["~standard"] as unknown).async = true;
        }
        (ctx as unknown).common = {
          issues: [],
          async: true,
        };
      }
  ...
  optional(): ZodOptional<this> {
    return ZodOptional.create(this, this._def) as unknown;
  }
  nullable(): ZodNullable<this> {
    return ZodNullable.create(this, this._def) as unknown;
  }
  ...
  or<T extends ZodTypeAny>(option: T): ZodUnion<[this, T]> {
    return ZodUnion.create([this, option], this._def) as unknown;
  }
  ...
  describe(description: string): this {
    const This = (this as unknown).constructor;
    return new This({
      ...this._def,
      description,
    });
...
> {
  _parse(input: ParseInput): ParseReturnType<this["_output"]> {
    ...
    if (ctx.common.async) {
      return Promise.all(
        ([...ctx.data] as unknown[]).map((item, i) => {
    ...
    const result = ([...ctx.data] as unknown[]).map((item, i) => {
  ...
  nonempty(message?: errorUtil.ErrMessage): ZodArray<T, "atleastone"> {
    return this.min(1, message) as unknown;
  }
...
function deepPartialify(schema: ZodTypeAny): any {
  if (schema instanceof ZodObject) {
    ...
    return ZodTuple.create(schema.items.map((item: unknown) => deepPartialify(item)));
  } else {
    return schema;
  }
}
...
> extends ZodType<Output, ZodObjectDef<T, UnknownKeys, Catchall>, Input> {
  ...
  _parse(input: ParseInput): ParseReturnType<this["_output"]> {
    ...
    if (ctx.common.async) {
      ...
      return ParseStatus.mergeObjectSync(status, pairs as unknown);
    }
  }
  ...
  merge<Incoming extends AnyZodObject, Augmentation extends Incoming["shape"]>(
    ...
    const merged: unknown = new ZodObject({
  ...
  setKey<Key extends string, Schema extends ZodTypeAny>(
    ...
    return this.augment({ [key]: schema }) as unknown;
  }
    ...
    for (const key of util.objectKeys(this.shape)) {
      if (mask && !mask[key]) {
        ...
        while (newField instanceof ZodOptional) {
          newField = (newField as ZodOptional<unknown>)._def.innerType;
        }
  ...
  keyof(): ZodEnum<enumUtil.UnionToTupleString<keyof T>> {
    return createZodEnum(util.objectKeys(this.shape) as [string, ...string[]]) as unknown;
  }
...
> {
  _parse(input: ParseInput): ParseReturnType<this["_output"]> {
    ...
    function handleResults(results: { ctx: ParseContext; result: SyncParseReturnType<unknown> }[]) {
    ...
    if (ctx.common.async) {
      ...
      let dirty: undefined | { result: DIRTY<unknown>; ctx: ParseContext } = undefined;
...
> extends ZodTypeDef {
  ...
  options: Options;
  optionsMap: Map<Primitive, ZodDiscriminatedUnionOption<unknown>>;
...
function mergeValues(a: unknown, b: unknown): { valid: true; data: any } | { valid: false } {
...
> {
  _parse(input: ParseInput): ParseReturnType<this["_output"]> {
    ...
    if (ctx.common.async) {
      ...
      ]).then(([left, right]: unknown) => handleParsed(left, right));
    } else {
      return handleParsed(
  ...
  _parse(input: ParseInput): ParseReturnType<this["_output"]> {
    ...
    const items = ([...ctx.data] as unknown[])
      .map((item, itemIndex) => {
        ...
        if (!schema) return null as unknown as SyncParseReturnType<unknown>;
...
> {
  ...
  static create(first: unknown, second?: unknown, third?: unknown): ZodRecord<any, any> {
...
> {
  _parse(input: ParseInput): ParseReturnType<this["_output"]> {
    ...
    function finalizeSet(elements: SyncParseReturnType<unknown>[]) {
  ...
  size(size: number, message?: errorUtil.ErrMessage): this {
    return this.min(size, message).max(size, message) as unknown;
  }
...
> = Args["_input"] extends Array<unknown> ? (...args: Args["_input"]) => Returns["_output"] : never;

export type InnerTypeOfFunction<
  Args extends ZodTuple<any, any>,
  Returns extends ZodTypeAny,
> = Args["_output"] extends Array<unknown> ? (...args: Args["_output"]) => Returns["_input"] : never;
...
> {
  _parse(input: ParseInput): ParseReturnType<unknown> {
    ...
    if (this._def.returns instanceof ZodPromise) {
      ...
      const me = this;
      return OK(async function (this: unknown, ...args: unknown[]) {
        const error = new ZodError([]);
        const parsedArgs = await me._def.args.parseAsync(args, params).catch((e) => {
          error.addIssue(makeArgsIssue(args, e));
          throw error;
        });
        const result = await Reflect.apply(fn, this, parsedArgs as unknown);
      ...
      // eslint-disable-next-line @typescript-eslint/no-this-alias
      const me = this;
      return OK(function (this: unknown, ...args: unknown[]) {
  ...
  args<Items extends Parameters<(typeof ZodTuple)["create"]>[0]>(
    ...
    return new ZodFunction({
      ...this._def,
      args: ZodTuple.create(items).rest(ZodUnknown.create()) as unknown,
    });
  }
  ...
  implement<F extends InnerTypeOfFunction<Args, Returns>>(
    ...
    return validatedFunc as unknown;
  }
  ...
  static create(args?: AnyZodTuple, returns?: ZodTypeAny, params?: RawCreateParams) {
    return new ZodFunction({
      args: (args ? args : ZodTuple.create([]).rest(ZodUnknown.create())) as unknown,
...
> {
  ...
  static create = <Inner extends ZodTypeAny>(
    ...
    return new ZodDefault({
      innerType: type,
      typeName: ZodFirstPartyTypeKind.ZodDefault,
      defaultValue: typeof params.default === "function" ? params.default : () => params.default as unknown,
      ...processCreateParams(params),
    }) as unknown;
  };
}
...
///////////////////////////////////////////
type BuiltIn =
  | (((...args: unknown[]) => any) | (new (...args: unknown[]) => any))
...
export function custom<T>(
  ...
  _params: string | CustomParams | ((input: unknown) => CustomParams) = {},
  /**
   * @deprecated
   *
...
const instanceOfType = <T extends typeof Class>(
  // const instanceOfType = <T extends new (...args: unknown[]) => any>(
  cls: T,
  params: CustomParams = {
