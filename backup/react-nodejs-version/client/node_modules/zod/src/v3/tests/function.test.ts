...
test("function parsing", () => {
  const parsed = func1.parse((arg: unknown) => arg.length);
  parsed("asdf");
});
...
test("parsed function fail 2", () => {
  const parsed = func1.parse((x: string) => x);
  expect(() => parsed(13 as unknown)).toThrow();
});
...
test("input validation error", () => {
  ...
  const checker = () => {
    invalidFuncInstance("Invalid_input" as unknown);
  };
...
test("output validation error", () => {
  const invalidFuncInstance = func2.validate((_x) => {
    return ["this", "is", "not", "valid", "output"] as unknown;
  });
...
test("special function error codes", () => {
  const checker = z.function(z.tuple([z.string()]), z.boolean()).implement((arg) => {
    return arg.length as unknown;
  });
  try {
    checker("12" as unknown);
  } catch (err) {
