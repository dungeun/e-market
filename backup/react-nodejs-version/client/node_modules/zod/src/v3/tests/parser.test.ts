...
test("parse strict object with unknown keys", () => {
  expect(() =>
    z
      ...
      .strict()
      .parse({ name: "bill", unknownKey: 12 } as unknown)
  ).toThrow();
});
...
test("invalid left side of intersection", () => {
  expect(() => z.intersection(z.string(), z.number()).parse(12 as unknown)).toThrow();
});
...
test("parsing non-array in tuple schema", () => {
  expect(() => z.tuple([]).parse("12" as unknown)).toThrow();
});

test("incorrect num elements in tuple", () => {
  expect(() => z.tuple([]).parse(["asdf"] as unknown)).toThrow();
});

test("invalid enum value", () => {
  expect(() => z.enum(["Blue"]).parse("Red" as unknown)).toThrow();
});
