...
const fetchLanguageSections = async (language: LanguageCode): Promise<UILocalizationData> => {
  try {
    // API에서 데이터 가져오기 (DB 기반)
    const response = await fetch(`/api/home/sections?lang=${language}&t=${Date.now()}`, {
      method: 'GET',
      headers: {
    ...
    if (apiResponse.sections && Array.isArray(apiResponse.sections)) {
      ...
      apiResponse.sections.forEach((section: any) => {
        // section.key를 우선 사용, 없으면 type을 사용
        const sectionKey = section.key || section.type;
        sectionOrder.push(sectionKey);
        sections[sectionKey] = {
          id: sectionKey,
          type: section.type,
          visible: section.isActive !== false,
    ...
    const sections: Record<string, UISection> = {};
    if (rawData.sections && typeof rawData.sections === 'object') {
    ...
    const result: UILocalizationData = {
      version: rawData.version || '2025.9.2',
...
export function useOptimizedUILocalization(
  ...
  const getSectionData = useMemo(() => {
    return (sectionId: string): unknown => {
